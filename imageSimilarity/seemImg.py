...
def doSimilarity_classify_gray_hist(image1Path,image2Path):
    ...
    print ("基于灰度化直方图相似度:").decode('utf-8').encode('gbk'),degree
    return degree

def doSimilarity_classify_hist_with_split(image1Path,image2Path):
    sm=similarityWorker(image1Path,image2Path)
    degree = sm.classify_hist_with_split()
    print ("分离各通道直方图后的相似度:").decode('utf-8').encode('gbk'),degree
    return degree
...
#     doSimilarity_classify_gray_hist("38.jpg","lena.jpg");
#     doSimilarity_classify_aHash("38.jpg","lena.jpg");
